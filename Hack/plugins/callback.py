...
async def e(e):
    async with bot.conversation(e.chat_id) as x:
        ...
        grp_name = await ask_id(
            x, text='Please send Group/Channel Username or Invite Link⚡️𝐌 𝐎 𝐎 𝐍⚡️')
        if not grp_name:
            return
...
async def f(e):
    async with bot.conversation(e.chat_id) as x:
        ...
        if not string:
            return
        grp_name = await ask_id(
            x, text='Please send Group/Channel Username or ID⚡️𝐌 𝐎 𝐎 𝐍⚡️')
        if not grp_name:
            return
...
async def l(e):
    async with bot.conversation(e.chat_id) as x:
        await x.send_message("Choose the Broadcast Type⚡️𝐌 𝐎 𝐎 𝐍⚡️",
...
async def n(e):
    async with bot.conversation(e.chat_id) as x:
        ...
        from_grp = await ask_id(
            x, text='Please Send The Group Id You Want Scrap From⚡️𝐌 𝐎 𝐎 𝐍⚡️')
        if not from_grp:
            return

        to_grp = await ask_id(
            x, text='Please Send The Group Id You Want Scrap To⚡️𝐌 𝐎 𝐎 𝐍⚡️')
        if not to_grp:
            return
...
async def o(e):
    async with bot.conversation(e.chat_id) as x:
        ...
        user_id = await ask_id(x, text='Please send the user id/username⚡️𝐌 𝐎 𝐎 𝐍⚡️')
        if not user_id:
            return
