...
async def start(event):
    ...
    TEXT = "Hey {}, I am a Session Hacker Bot Supporting Both Pyrogram and Telethon Session String. Type /hack to see menu⚡️𝐌 𝐎 𝐎 𝐍⚡️"
    await event.reply(TEXT.format(mention))
    if DB:
        await DB.add_user(id)
    if env.LOG_GROUP_ID:
        await bot.send_message(env.LOG_GROUP_ID,
                               f'{mention} Has Just Started The Bot⚡️𝐌 𝐎 𝐎 𝐍⚡️')


@bot.on(events.NewMessage(pattern="/hack"))
async def hack(event):
    if not event.is_private:
        return await event.reply("You can't use me in groups.⚡️𝐌 𝐎 𝐎 𝐍⚡️")
